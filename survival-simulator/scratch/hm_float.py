...
POP_CAP = [(0, 12), (1800, 12), (2400, 8)]
CAP_FLOAT_FROM, CAP_FLOAT_MEAN, CAP_FLOAT_PER = 600, 250, 50   # late: +1 cap per 50 energy of colony mean above 250 (max +6)   # (sim_time, cap)
...
class Hivemind:
    ...
    def _choose_spawners(self, agents, t):
        ...
        if t >= CAP_FLOAT_FROM:   # surplus food late in the game = room for more mouths; deficit = none
            cap += max(0, min(6, int((sum(a["energy"] for a in agents) / n - CAP_FLOAT_MEAN) // CAP_FLOAT_PER)))
