...
class Hivemind:
    ...
    def _open_view(self, a, m):
        ...
        tree = min(((math.hypot(tr[0] - m["x"], tr[1] - m["y"]), tr) for tr in self.trees), default=None)
        if tree and tree[0] < 80:   # keep the home tree in the cone: its fruit must stay watched (ripe dating)
            c, span = math.atan2(tree[1][1] - m["y"], tree[1][0] - m["x"]), half - 0.15
            cands = [c + span * (k / 3 - 1) for k in range(7)]
        else:
            cands = [k * math.pi / 6 for k in range(12)]
        best = None
        for ang in cands:
