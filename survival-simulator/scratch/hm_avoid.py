...
AVOID_R, AVOID_FRESH = 280, 30   # a predator the map places this close (seen by anyone this recently): face it, step away
RIPEN, HUNGRY = 200, 0.25     # a fruit is worth 20 energy when it spawns and 60 after 20 s: wait unless below HUNGRY of max
...
class Hivemind:
    ...
    def _avoid(self, a, m, fruits):
        """Predators see 250 px, we see 200: use the colony's eyes. A fresh map predator inside AVOID_R that we do not
        see ourselves: turn to face it (a predator only charges what looks away) and walk off. Fruit underfoot first."""
        if m["err"] > LOC_OK or not self.predators or any(f["distance"] < 40 for f in fruits):
            return None
        px, py, pt = min(self.predators, key=lambda q: math.hypot(q[0] - m["x"], q[1] - m["y"]))
        d = math.hypot(px - m["x"], py - m["y"])
        if d > AVOID_R or self.tick - pt > AVOID_FRESH:
            return None
        toward = wrap(math.atan2(py - m["y"], px - m["x"]) - m["h"])
        m["mode"], m["scan_left"] = "avoid", 0
        return a["speed"], self._flee_dir(m, wrap(toward + math.pi)), toward
