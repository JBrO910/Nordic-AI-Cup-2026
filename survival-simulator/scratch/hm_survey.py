...
class Hivemind:
    ...
    def _update_world(self, agents):
        ...
        self.trees = [tr for tr in self.trees if self.tick - tr[2] < 900]
    ...
    def _forage(self, a, m, obs, edges):
        ...
        if self.tick - m["fruit_tick"] > (60 if self.tick >= 6000 else HOP_AFTER) and a["energy"] > HOP_MIN_ENERGY:
    ...
    def _relocate_to_tree(self, a, m):
        ...
        if near_tree and self.tick - m["fruit_tick"] < (60 if self.tick >= 6000 else HOP_AFTER):
            return None
        best = None
        for tr in self.trees:
        ...
        if m["err"] <= LOC_OK:
            ...
            for i in range(W // CELL + 1):
                for j in range(H // CELL + 1):
                    ...
                    if d < 100 or d > (900 if self.tick >= 6000 else 600):
                        continue
