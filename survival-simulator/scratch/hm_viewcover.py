...
SCAN_EVERY_ALERT = 10
VIEW_EVERY = 100         # ticks between re-choosing the open-view heading while sitting
...
COVER_VALUE, COVER_R = 150, 250   # from t=600 a fully shielded tree is worth this much walking; predator vision radius
...
class Hivemind:
    ...
    def reset(self):
        ...
        self.cover = {}       # tree cell -> (tick, exposure)
    ...
    def _scan_step(self, a, m):
        ...
        m["scan_left"] -= 1
        if m["scan_left"] == 0:
            m["view_tick"] = -10**9   # scan done: re-choose where to look
        m["mode"] = "scan"
    ...
    def _relocate_to_tree(self, a, m):
        ...
        for tr in self.trees:
            ...
            if any(math.hypot(tr[0] - om["x"], tr[1] - om["y"]) < self._spread() for oid, om in self.mem.items()
                ...
                continue
            score = d - GROVE_VALUE * self._grove_value(tr) + (COVER_VALUE * self._exposure(tr) if self.tick >= 6000 else 0)
    ...
    def _exposure(self, tr):
        """Share of 24 directions from which a predator could see the tree from COVER_R away (cached 300 ticks)."""
        key = (round(tr[0] / 10), round(tr[1] / 10))
        c = self.cover.get(key)
        if c is None or self.tick - c[0] > 300:
            open_ = sum(1 for k in range(24) if self._open_dist(tr[0], tr[1], k * math.pi / 12, COVER_R) >= COVER_R)
            c = self.cover[key] = (self.tick, open_ / 24)
        return c[1]
    ...
    def _sit(self, a, m, t):
        ...
        if m["err"] <= LOC_OK and self.predators:
            ...
            if math.hypot(px - m["x"], py - m["y"]) < 450:
                ...
                if abs(rel) > 0.25:
                    return 0.0, 0.0, rel
        if m["err"] <= LOC_OK:   # no threat known: watch the open side, a wall needs no watching
            if self.tick - m.get("view_tick", -10**9) > VIEW_EVERY:
                m["view_tick"], m["view_h"] = self.tick, self._open_view(a, m)
            rel = wrap(m["view_h"] - m["h"])
            if abs(rel) > 0.25:
                return 0.0, 0.0, rel
        return 0.0, 0.0, 0.0

    def _open_view(self, a, m):
        """Absolute heading whose cone sees the most open ground (12 headings x 3 rays against known edges)."""
        half, vr = a["vision_angle"] / 2, a["vision_range"]
        best = None
        for k in range(12):
            ang = k * math.pi / 6
            score = sum(self._open_dist(m["x"], m["y"], ang + f * half, vr) for f in (-0.6, 0.0, 0.6))
            if best is None or score > best[0]:
                best = (score, ang)
        return best[1]
    ...
    def _open_dist(self, x, y, ang, limit):
        """Distance along the ray from (x, y) at absolute angle `ang` to the nearest learned obstacle edge or the map
        boundary, capped at `limit`. Unknown terrain counts as open."""
        dx, dy = math.cos(ang), math.sin(ang)
        best = limit
        for bx, by in ((WALL, None), (W - WALL, None), (None, WALL), (None, H - WALL)):   # boundary walls
            if bx is not None and abs(dx) > 1e-9:
                t = (bx - x) / dx
            elif by is not None and abs(dy) > 1e-9:
                t = (by - y) / dy
            else:
                continue
            if 0 < t < best:
        ...
        for segs in self.landmarks.values():
            for ax1, ay1, ax2, ay2, _ in segs:
                if abs(ax1 - x) > limit + 60 or abs(ay1 - y) > limit + 60:
                    continue
                ex, ey = ax2 - ax1, ay2 - ay1
                den = dx * ey - dy * ex
                if abs(den) < 1e-9:
                    continue
                t = ((ax1 - x) * ey - (ay1 - y) * ex) / den
                u = ((ax1 - x) * dy - (ay1 - y) * dx) / den
