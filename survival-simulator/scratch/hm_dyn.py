...
POP_CAP = [(0, 40)]      # no cap: births are throttled by RESERVE, i.e. by food   # (sim_time, cap)
...
CULL_MEAN, CULL_FROM, CULL_SHARE = 100, 1000, 8   # colony mean energy below this (after t=100 s): the oldest 1/8 stop eating
...
class Hivemind:
    ...
    def _mem(self, aid):
        ...
        if m is None:
            m = self.mem[aid] = dict(x=0.0, y=0.0, h=0.0, err=1e9, pred=None, pred_tick=-10**9, scan_left=5,
                                     ...
                                     aging=False, cull=False, wall_cooldown=0, sprinting=False, mode="",
    ...
    def _assign(self, agents):
        ...
        loc = [(a, self.mem[a["agent_id"]]) for a in agents
               ...
               and not self.mem[a["agent_id"]]["cull"]]
    ...
    def _choose_spawners(self, agents, t):
        ...
        if self.tick >= CULL_FROM and n >= 4 and sum(a["energy"] for a in agents) / n < CULL_MEAN:
            for a in sorted(agents, key=lambda a: a["age"], reverse=True)[:max(1, n // CULL_SHARE)]:
                self._mem(a["agent_id"])["cull"] = True   # too many mouths: the oldest starve so the young eat
        chosen = set()
        dumpers = []
        for a in agents:
            m = self._mem(a["agent_id"])
            ...
            if ((m["aging"] or m["cull"]) and a["energy"] > 100) or old:
