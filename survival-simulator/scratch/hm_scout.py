...
SCOUT_FROM, SCOUT_MIN_ENERGY, SCOUT_STALE = 3000, 300, 1500   # from t=300 one rich idle agent explores the whole map for forgotten groves
...
class Hivemind:
    ...
    def reset(self):
        ...
        self.scout = None     # agent_id of the current scout
        self.scout_target = None
        self.scout_cooldown = 0
    ...
    def decide(self, step):
        ...
        if self.scout not in alive:
            self.scout = None
        if self.scout is None and self.tick >= max(SCOUT_FROM, self.scout_cooldown) and len(agents) >= 6:
            rich = [a for a in agents if a["energy"] > SCOUT_MIN_ENERGY and a["age"] < 40 and self._mem(a["agent_id"])["err"] <= LOC_OK
                    and self._mem(a["agent_id"])["mode"] in ("sit", "scan", "")]
            if rich:
                a = max(rich, key=lambda a: a["energy"])
                target = self._stale_cell(self._mem(a["agent_id"]))
                if target:
                    self.scout, self.scout_target = a["agent_id"], target
    ...
    def _forage(self, a, m, obs, edges):
        ...
        scout = a["agent_id"] == self.scout
        if self.tick - m["fruit_tick"] > (30 if scout else HOP_AFTER) and a["energy"] > HOP_MIN_ENERGY:
            return self._explore(a, m, obs, edges, scout)
        return None
    ...
    def _stale_cell(self, m):
        """Stalest cell nobody has looked at for SCOUT_STALE ticks that is not known desert/river, anywhere on the map."""
        best = None
        for i in range(W // CELL + 1):
            for j in range(H // CELL + 1):
                ...
                if stale < SCOUT_STALE or self.terrain.get((int(cx // 40), int(cy // 40))) in ("desert", "river"):
                    continue
                if any(math.hypot(cx - om["x"], cy - om["y"]) < 300 for om in self.mem.values() if om["err"] <= LOC_OK):
                    continue   # somebody is already there
                score = 0.2 * math.hypot(cx - m["x"], cy - m["y"]) - min(stale, 6000)
                if best is None or score < best[0]:
                    best = (score, cx, cy)
        return best[1:] if best else None

    def _scout_step(self, a, m, edges):
        """The scout walks to its target cell (fleeing on the way if needed); arrival ends the mission and it forages there."""
        if a["agent_id"] != self.scout or m["err"] > LOC_OK:
            return None
        tx, ty = self.scout_target
        d = math.hypot(tx - m["x"], ty - m["y"])
        if d < 60:
            self.scout, self.scout_target, self.scout_cooldown = None, None, self.tick + 300
            m["fruit_tick"] = self.tick - HOP_AFTER   # look around here
            return None
        rel = wrap(math.atan2(ty - m["y"], tx - m["x"]) - m["h"])
        m["mode"], m["scan_left"] = "scout", 0
        return a["speed"], self._flee_dir(m, rel), rel if abs(rel) > 0.5 else 0.0

    def _explore(self, a, m, obs, edges, scout=False):
        ...
        if m["err"] <= LOC_OK:
            ...
            for i in range(W // CELL + 1):
                for j in range(H // CELL + 1):
                    ...
                    if d < 100 or (d > 600 and not scout):
                        continue
                    stale = self.tick - self.cell_seen.get((i, j), -10**6)
                    b = self.terrain.get((int(cx // 40), int(cy // 40)))
                    if scout and (stale < SCOUT_STALE or b in ("desert", "river")):
                        continue   # the scout only goes where nobody has looked for a while and trees can grow
