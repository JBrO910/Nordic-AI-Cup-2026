...
class Hivemind:
    ...
    def _flee(self, a, m, preds):
        ...
        if preds:
            ...
            if (m["aging"] or a["energy"] <= 0.2 * a["max_energy"] + 5) and m["err"] <= LOC_OK:
                # decoy: cannot sprint or dying anyway - lead the predator away from the colony, terrain be damned
                loc = [om for oid, om in self.mem.items() if oid != a["agent_id"] and om["err"] <= LOC_OK]
                if len(loc) >= 2:
                    hx, hy = sum(om["x"] for om in loc) / len(loc), sum(om["y"] for om in loc) / len(loc)
                    away_home = wrap(math.atan2(m["y"] - hy, m["x"] - hx) - m["h"])
                    away_pred = wrap(p["angle"] + math.pi)
                    ux, uy = math.cos(away_home) + math.cos(away_pred), math.sin(away_home) + math.sin(away_pred)
                    m["scan_left"], m["mode"] = 0, "decoy"
                    return speed, math.atan2(uy, ux), p["angle"] - 0.35
            if FLEE_NET is not None:
                m["scan_left"], m["mode"] = 0, "flee"
                dist, direction, turn = flee_net.act(FLEE_NET, a, (p["distance"], p["angle"], p["rel_dir"]), True, self._map_preds(m, preds))
                return dist, self._flee_dir(m, direction), turn   # the net is blind to terrain, walls and obstacles
            fx = fy = 0.0
            for o in preds:
        ...
        if m["pred"] is not None and self.tick - m["pred_tick"] < FLEE_MEMORY:
            ...
            if FLEE_NET is not None:
                ...
                dist, direction, turn = flee_net.act(FLEE_NET, a, (math.hypot(dx, dy), wrap(math.atan2(dy, dx) - m["h"]), m["pred_obs"][2]), False, self._map_preds(m, preds))
                return dist, self._flee_dir(m, direction), turn
