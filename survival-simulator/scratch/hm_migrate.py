...
MIG_FROM, MIG_EVERY, MIG_COOLDOWN, MIG_MIN_D, MIG_ENERGY = 3000, 300, 3000, 400, 150   # colony migration (see _plan_migration)
MIG_HUNGRY, MIG_MIN_YIELD, MIG_PARTY = 0.5, 1.2, 4
BIOME_PRIOR = {"forest": 2.0, "swamp": 1.5, "grassland": 1.0, None: 0.8, "desert": 0.2, "river": 0.0}   # expected fruiting trees in an unseen cell
...
class Hivemind:
    ...
    def reset(self):
        ...
        self.mig_target, self.mig_until, self.mig_last = None, 0, -10**9   # colony migration: target, join window, last start
        self.cell_peak = {}   # grid cell -> most trees ever known there at once (trees regrow where they grew)
        self.cell_spot = {}   # grid cell -> where those trees stood
    ...
    def _mem(self, aid):
        ...
        if m is None:
            m = self.mem[aid] = dict(x=0.0, y=0.0, h=0.0, err=1e9, pred=None, pred_tick=-10**9, scan_left=5,
                                     ...
                                     aging=False, migrate=None, wall_cooldown=0, sprinting=False, mode="",
    ...
    def decide(self, step):
        ...
        if self.tick >= MIG_FROM and self.tick % MIG_EVERY == 0:
            self._plan_migration(agents)
    ...
    def _update_world(self, agents):
        ...
        if self.tick % 50 == 0:   # remember how many trees each cell has held at once: where trees grew, they regrow
            counts = {}
            for tr in self.trees:
                k = (int(tr[0] // CELL), int(tr[1] // CELL)); counts[k] = counts.get(k, 0) + 1
            for k, c in counts.items():
                if c >= self.cell_peak.get(k, 0):
                    self.cell_peak[k] = c
                    xs = [tr for tr in self.trees if (int(tr[0] // CELL), int(tr[1] // CELL)) == k]
                    self.cell_spot[k] = (sum(tr[0] for tr in xs) / c, sum(tr[1] for tr in xs) / c)
    ...
    def _assign(self, agents):
        ...
        loc = [(a, self.mem[a["agent_id"]]) for a in agents
               ...
               and not self.mem[a["agent_id"]]["migrate"]]
        ...
        for d, aid, i in pairs:
            ...
            if not hungry and not self._ripe(i, self.tick + d / 10):
                continue
            self.claims[aid] = i
            taken.add(i)
    ...
    def _eat(self, a, m, fruits, edges):
        ...
        if m["aging"] and not (nearest is not None and nearest < 60):
            m["mode"] = "aging"
            return 0.0, 0.0, 0.0
        if m["migrate"] and not (nearest is not None and nearest < 40):
            return None
    ...
    def _plan_migration(self, agents):
        """Every 30 s: a hungry colony (mean energy below MIG_HUNGRY of max) with an unoccupied region of decent yield on
        the map (seen trees, or biome priors for unseen cells) sends an expedition of its MIG_PARTY richest healthy
        agents to hold it. Cooldown keeps the colony from bleeding parties."""
        loc = [(a, self._mem(a["agent_id"])) for a in agents if self._mem(a["agent_id"])["err"] <= LOC_OK]
        if self.tick - self.mig_last < MIG_COOLDOWN or len(loc) < 6:
            return
        if sum(a["energy"] / a["max_energy"] for a in agents) / len(agents) > MIG_HUNGRY:
            return
        density = min(1.0, len(self.trees) / 30)   # trees decay through the game: scale the priors
        best = None
        for i in range(W // CELL + 1):
            for j in range(H // CELL + 1):
                cx, cy = min(i * CELL + CELL / 2, W - 60), min(j * CELL + CELL / 2, H - 60)
                if any(math.hypot(cx - m["x"], cy - m["y"]) < MIG_MIN_D for _, m in loc):
                    continue   # not unoccupied
                if self.tick - self.cell_seen.get((i, j), -10**6) < 600:
                    y = self._grove_value((cx, cy))
                else:
                    y = max(BIOME_PRIOR.get(self.terrain.get((int(cx // 40), int(cy // 40))), 0.8), 0.7 * self.cell_peak.get((i, j), 0)) * density
                if best is None or y > best[0]:
                    sx, sy = self.cell_spot.get((i, j), (cx, cy))
                    best = (y, sx, sy)
        if not best or best[0] < MIG_MIN_YIELD:
            return
        party = sorted((a for a, m in loc if a["energy"] > MIG_ENERGY and a["age"] < 40 and not m["aging"] and not m["mode"].startswith("flee")),
                       key=lambda a: a["age"])[:MIG_PARTY]   # the young carry the colony's future, not the old
        if len(party) < 3:
            return
        self.mig_target, self.mig_until, self.mig_last = (best[1], best[2]), self.tick + 300, self.tick
        for a in party:
            self._mem(a["agent_id"])["migrate"] = self.mig_target

    def _migrate_step(self, a, m):
        """Walk to the colony's migration target; arrival (or losing localization) ends this agent's trip."""
        if not m["migrate"]:
            return None
        if m["err"] > LOC_OK:
            m["migrate"] = None
        ...
        if d < 150:
            ...
            return None
        rel = wrap(math.atan2(ty - m["y"], tx - m["x"]) - m["h"])
        m["mode"], m["scan_left"], m["hop_until"] = "migrate", 0, 0
        return a["speed"], self._flee_dir(m, rel), rel if abs(rel) > 0.5 else 0.0
