...
def fitness(a):
    """Who breeds. Children mutate ±50 % per trait (10 % chance each), so this is selective breeding over the game:
    sprint speed (cap 40; predators sprint 15) and vision (cap 400; predators see 250) decide encounters.
    max_energy is penalised: the sprint lock is 20 % *of max*, so a big tank cannot sprint at late-game energy levels."""
    return (3 * a["sprint_speed"] / 40 + a["speed"] / 20 + 1.5 * a["vision_range"] / 400
            + a["vision_angle"] / (math.pi / 2) + a["hearing_radius"] / 100 - a["max_energy"] / 1000)
...
class Hivemind:
    ...
    def _choose_spawners(self, agents, t):
        ...
        if len(agents) >= 6:   # selection pressure: the weaker half of the colony does not pass its traits on by dumping
            median = sorted(fitness(a) for a in agents)[len(agents) // 2]
            dumpers = [a for a in dumpers if fitness(a) >= median]
        dumpers.sort(key=fitness, reverse=True)
